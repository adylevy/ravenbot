...
function Bot (config) {
 ...
 if (this.token && this.group && this.adminName) {
    console.log("registering the bot in admin room");
    this.registerBot(this.group,this.adminName);
  } else {
...
Bot.prototype.killAllBots = function(callback) {
  this.allBots(function(listOfBots) {
    _.each(listOfBots, function(bot) {
      this.emit('botInternalMsg',this,bot);
...
Bot.prototype.registerBot = function(groupId,botName) {
  async.waterfall([
      ...
      this.allBots(function(bots) {
       // console.log('all bots:',bots);
        ...
        _.each(bots, function(bot) {
          if (bot.group_id == groupId && bot.name == botName) {
